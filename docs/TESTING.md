...
## CURRENT — Canonical round advancement (M2N)

`src/application/tournamentCommands.js`'s `advanceTournamentRound(applicationState, { tableNumbers })` is the canonical application-layer equivalent of current legacy `nextRound()`, verified against direct source inspection (see that command's own extensive header comment for the full characterization, including a discovered discrepancy against this task's own suggested `reconstructStandingsBeforeRound()` hint — legacy `nextRound()` does not call it; see docs/ARCHITECTURE.md). It requires a real RUNNING tournament, every current-round match `done` or `cancelled` (a missing current round is rejected, not treated as vacuously complete like legacy's own `|| []` fallback would), and `currentRound < totalRounds`. **RECALCULATION != RE-PAIRING**: it generates ONLY `rounds[currentRound + 1]` via `generatePairings()` (no BBS/pairing formula changes) and never touches any existing round. Standings are computed via an internal `recalculateTournamentPlayers()` call first — a canonical-world necessity, not a legacy behavior change, since there is no automatic recalculation effect here (matters for a bye-only round, which `recordMatchResult()` never touches). Pending players are promoted using legacy's exact closest-avgPerf-splice algorithm, stamped with `joinedRound`, in the same minimal transient shape; `pendingPlayers` is cleared on success. A `preAdvanceSnapshot` is captured in the exact established legacy shape. Ordering is atomic and legacy-correct: pairings are computed before any state changes, exactly like `nextRound()`'s own "do not mutate until pairings are legal" comment (unlike `startTournament()`'s discovered ordering bug). `src/application/tournamentStore.js` exposes this as `store.advanceTournamentRound(args)`.

`tests/advance-tournament-round.test.js` covers: a completed Round 1 advancing to Round 2 with `currentRound` incrementing; prior rounds remaining deep-equal; a hand-verified deterministic Round 2 pairing (Swiss/cost-based, distinct MP/PERF values); incomplete-round rejection; a cancelled match counting as "complete" for the gate; a bye counting as already-done (and correctly reflecting the corrected canonical bye rule — MP but no games); pending-player promotion (`joinedRound`, transient shape, inclusion in Round 2 pairing, `pendingPlayers` cleared); table numbering (default sequential, explicit `tableNumbers`); full `preAdvanceSnapshot` shape/values; **full `preAdvanceSnapshot` DETACHMENT (Codex follow-up fix)** — `preAdvanceSnapshot.allRounds`/`.tournament.players`/`.pendingPlayers` are proven, via direct mutation of the returned/snapshot objects afterward (not mere value equality at capture time), to share no references with each other, the returned `tournament`, or the original command input, matching legacy's own `JSON.parse(JSON.stringify(...))` capture; final-round rejection; EMPTY/pre-start/missing-round rejection; full input immutability including on failure; fixed-rack and 14.1 Round 2 generation; three sequential advances (Round 1 → 2 → 3) each touching only the newly generated round; the canonical-world recalculation-necessity proof (a round completed without an intervening `recordMatchResult()` call still produces correct standings); a naturally-reachable (not artificial) no-legal-bye pairing failure — a fixed 3-player headcount produces a rotating bye across three rounds until every player has had one, making the 4th round's bye illegal per whitepaper §9.2 — proven atomic at both the pure-command and store level (input, `pendingPlayers`, and an existing prior `preAdvanceSnapshot` all left completely unchanged); and store-level composition/atomicity — 30 tests.

`tests/historical-14-1-replay.test.js` runs the real canonical chain (`startTournament(cross_elo)` → `recordMatchResult()` for every actual Round 1 result → `advanceTournamentRound()`) against the `vm-joes-14-1` fixture: Round 1 pairings match historical exactly (5/5), and Round 1's recalculated PERF values match the historical per-match PERF values exactly for all 10 players (confirming `d: 330` and the metadata-documented 14.1 weights are correct). Round 2 is the first divergent round — 3 of 5 player pairings match exactly; the remaining divergence (a tier-target boundary case and a single-point PERF-gap tie-break) is asserted and characterized explicitly rather than hidden, with the likely cause (remaining config/rounding uncertainty at tight numerical boundaries, per the fixture's own documented `knownConfigurationUncertainty`) — no formula was tuned to force a match — 3 tests.

## CURRENT — Legacy ↔ canonical workflow integration (M2O)

Proves the full canonical command chain executes correctly through BOTH directions of the legacy compatibility boundary — `captureApplicationStateFromLegacy() → startTournament() → recordMatchResult() → advanceTournamentRound() → (repeat) → projectApplicationStateToLegacy()` — composed entirely from existing, already-unit-tested exports. No new production code was added; this milestone is integration tests and documentation only, catching the class of error isolated unit tests cannot (field-name mismatches between what a command writes and what the projector reads, cross-round reference leaks, canonical-only fields leaking into the projected legacy shape).

`tests/workflow-orchestration.test.js` (synthetic fixed-rack) covers: a legacy-shaped `CONFIGURED_PRE_START` input captured into canonical; `startTournament()`; every Round 1 match scored via `recordMatchResult()`; `advanceTournamentRound()` to Round 2; every Round 2 match scored; a second `advanceTournamentRound()` to Round 3; a correction to the *original* Round 1 result after Rounds 2–3 already exist, proving both later rounds and the standing `preAdvanceSnapshot` remain untouched; a string table identifier (`assignMatchTable()`) surviving both the correction and projection; full parity on projection (`config`/`tournamentConfig`/`tournament.players`/`totalRounds`/`allRounds`/`currentRound`/`pendingPlayers`/`preAdvanceSnapshot`, with no canonical-only field such as `started` leaking into the projected legacy `tournament` object); and a re-capture of the projected legacy state reproducing the same canonical rounds/currentRound/players. A second test covers a pending player joining mid-tournament (registered in both roster and `pendingPlayers`, matching the characterized legacy duplication), promoted correctly on advance, and surviving projection — 2 tests.

`tests/workflow-orchestration-historical-14-1.test.js` drives the real `vm-joes-14-1` event through the full boundary as far as already-characterized evidence allows: legacy-shaped capture → `startTournament(cross_elo)` (Round 1 pairing re-verified exact through the full boundary) → real Round 1 results entered via `recordMatchResult()` → `advanceTournamentRound()` to Round 2 (re-confirms the 3-of-5 exact-pairing characterization from `tests/historical-14-1-replay.test.js`) → Round 2 results entered as REAL data for the 3 exactly-matching pairings and clearly-labeled SYNTHETIC data for the 2 pairings with no historical equivalent this round (the already-characterized tier-target-boundary and PERF-tie-break divergence — not re-litigated or forced here) → `advanceTournamentRound()` to Round 3 (chain-mechanics proof only, not claimed historically accurate, since Round 2 already diverged) → `projectApplicationStateToLegacy()`, verifying the same parity checks as the synthetic test plus 14.1 target survival and a projected-then-recaptured round trip — 1 test.

`npm test` currently passes 381/381 and `npm run build` passes.
