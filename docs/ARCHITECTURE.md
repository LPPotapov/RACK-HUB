...
TARGET: a future application command will make the explicit, controlled `started: false → true` transition (starting Round 1), analyzed against legacy `startTournament()` behavior — not implemented in M2K-A. TARGET: the future canonical → legacy compatibility projection (not implemented yet — see M2H/M2K-A notes above) will read `Tournament.started` directly rather than re-deriving the EMPTY/CONFIGURED_PRE_START/RUNNING distinction heuristically from player/round counts.
