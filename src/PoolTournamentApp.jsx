...
const PoolTournamentApp = () => {
  ...
  // Read the active straight-pool config with safe defaults (old snapshots may lack it)
  const getSP = () => ({ ...STRAIGHT_POOL_DEFAULTS, ...(config.straightPool || {}) });
  // True when the tournament is running in 14.1 experimental mode
  const isStraightPool = () => config.format === 'straight_pool_14_1';
  ...
  const calcStraightPoolSignals = (p1Points, p2Points, innings, p1HighRun, p2HighRun, target, weights) => {
  ...
  const calcStraightPoolPerf = (opponentGbr, s141) => {
    if (s141 >= 0.999) return opponentGbr + 3 * config.d;
    if (s141 <= 0.001) return opponentGbr - 3 * config.d;
    return Math.round(opponentGbr + config.d * Math.log10(s141 / (1 - s141)));
  };
  ...
  const calcNPD = (pointsFor, pointsAgainst, target) => {
    const T = target > 0 ? target : 1;
